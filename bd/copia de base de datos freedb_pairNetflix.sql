...
-- Crear tabla rel_movies_users // Esto es lo mismo que hicimos con el diagrama, pero con código:
...
INSERT INTO rel_movies_users (idMoviesUsers, fkMovie, fkUser)
VALUES 	(1, 1, 1),	-- La usuaria con id 1 tiene como favoritas las películas con id 1 y 2.
		(2,	1, 1),
        (3, 2, 2);	-- La usuaria con id 2 tiene como favorita solo la película con id 2

SELECT users.idUser, users.name, movies.idMovies, movies.title
FROM users 
INNER JOIN rel_movies_users
ON users.idUser = rel_movies_users.fkUser
INNER JOIN movies
ON movies.idMovies = rel_movies_users.fkMovie;

SELECT * FROM rel_movies_users;

-- Falta Oppenheimer y cambiar John Travolta en Pulp Fiction y Roberto Benini en La vita è bella
SELECT actors.idActor, actors.name, actors.lastname, movies.idMovies, movies.title AS "movie where appears"
FROM actors
INNER JOIN rel_movies_actors
ON actors.idActor = rel_movies_actors.fkActor
INNER JOIN movies
ON movies.idMovies = rel_movies_actors.fkMovie;
